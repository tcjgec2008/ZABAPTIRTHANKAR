...
*----------------------------------------------------------------
  ...
  DATA lo_task_wreq TYPE REF TO cl_stct_create_req_wbenc_fiori.
  ...
  READ TABLE ir_tasklist->ptx_task INTO ls_task_wreq WITH KEY taskname = 'CL_STCT_CREATE_REQ_WBENC_FIORI'.
  ...
  IF sy-subrc = 0.
    TRY.
      ...
      CATCH cx_sy_move_cast_error INTO lx_cast_exc_wreq.
        MESSAGE e000 WITH 'Could not get Request from Task CREATE WORKBECNH REQUEST' INTO if_stctm_task~pr_log->dummy ##MG_ARG_LEN ##NO_TEXT.
...
*
  ...
  IF i_check EQ 'X' . "check mode
    ...
    READ TABLE ir_tasklist->ptx_task INTO ls_task WITH KEY taskname = 'CL_STCT_CREATE_REQ_WBENC_FIORI'.
  ...
  ELSE. " execution mode
    ...
    SELECT COUNT( * ) FROM /ui2/c_sysalias UP TO 1 ROWS WHERE sys_alias = lv_cusal.
    ...
    " check if entry exists
    SELECT COUNT( * ) FROM /ui2/sysaliasmap UP TO 1 ROWS WHERE sysalias_src = lv_mapal.
