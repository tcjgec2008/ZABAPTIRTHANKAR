  METHOD if_stctm_bg_task~execute.
    ...
    DATA lv_alias_taskproc TYPE c LENGTH 16.
...
*****************************************
    ...
    DATA lo_task TYPE REF TO cl_stct_fcm_determine_services.
    ...
    READ TABLE ir_tasklist->ptx_task INTO ls_task WITH KEY taskname = 'CL_STCT_FCM_DETERMINE_SERVICES'.

    IF sy-subrc = 0.
      TRY.
          lo_task ?= ls_task-r_task.
          lt_services = lo_task->it_services.
    ...
    LOOP AT lt_services INTO DATA(ls_service).
      ...
      ls_services_activation-selected = 'X'.
      ls_services_activation-service = ls_service-service.
      ls_services_activation-version = ls_service-version.

      APPEND ls_services_activation TO lt_services_activation.
...
*----------------------------------------------------------------
    LOOP AT pt_variant INTO ls_variant.
      CASE  ls_variant-selname.
        ...
        WHEN 'P_SELDES'.
          lv_alias_taskproc  = ls_variant-low.
...
*****************************************
    ...
    ELSE. " execution mode
      ...
      IF lt_services_activation IS INITIAL.

        MESSAGE w000 WITH 'No OData Services defined'(101) INTO if_stctm_task~pr_log->dummy.
        if_stctm_task~pr_log->add_syst( ).
        EXIT.
...
*****************************************
      ...
      " for each service
      LOOP AT lt_services_activation INTO ls_services_activation.

        " clear
        CLEAR: lv_service_name, lv_service_version, lv_service_name_bep, lv_active, lv_service_id, lv_service_name_tech, lv_alias.
        ...
        "special handling for taskprocessing
        IF ls_services_activation-service = 'TASKPROCESSING'.
          lv_alias = lv_alias_taskproc.
          lv_processing_mode = /iwfnd/if_mgw_core_types=>gcs_process_mode-routing_based.
          ls_results-alias = lv_alias_taskproc.
        ENDIF.
          ...
          IF sy-subrc = 0.
            ...
            IF lv_processing_mode = /iwfnd/if_mgw_core_types=>gcs_process_mode-co_deployed_only.
                ...
                IF ls_services_data-process_mode <> 'C'.
                  ...
                  "additional check, if alias LOCAL is assigned
                  READ TABLE lt_sys_aliases WITH KEY system_alias = 'LOCAL' TRANSPORTING NO FIELDS.

                  IF sy-subrc = 0.
                    ls_results-status = 's'.
                    ls_results-servicestatus = 'diff. proc. mode'(123).
                    ls_results-alias = 'LOCAL'.
                    ls_results-statusalais = 's'.
                    ls_results-aliasstatus = 'found'(124).
                  ELSE.
                    ls_results-status = 'w'.
                    ls_results-servicestatus = 'diff. proc. mode'(123).
                  ENDIF.
...
*****************************************
          ...
          ELSE.
            ...
            IF lv_service_id  IS NOT INITIAL.
              ...
              " Check alias exists
              SELECT SINGLE * FROM /iwfnd/c_dfsyal INTO @DATA(ls_avail_alias) WHERE system_alias = @lv_alias.

              IF ls_avail_alias IS INITIAL.
                " alias is not available
                ls_results-statusalais = 'e'.
                ls_results-aliasstatus = 'not available'(110).
              ELSE.
...
***************** PREPARE LOGRESULTS *****************
      ...
      LOOP AT lt_results INTO ls_results.
        ...
        IF ls_results-alias IS INITIAL.
          CONCATENATE '/' ls_results-icfstitle ls_results-icfstatus INTO lv_str2 SEPARATED BY sep.
          lv_str3 = ''.
        ELSE.
          CONCATENATE '/' ls_results-aliastitle ls_results-alias ls_results-aliasstatus INTO lv_str3 SEPARATED BY sep.
        ENDIF.
        ...
        ELSEIF ls_results-status = 'e' OR ls_results-statusalais = 'e' OR ls_results-statusicf = 'e'.
          ...
          lv_details = lv_details && ls_results-servicename && ':' && ls_results-serviceversion && ';'.
      ...
      IF lv_raise_error =  abap_true.
        ...
        MESSAGE e000 WITH 'For detailed analysis activate failed service'  'manually with transaction' '/IWFND/MAINT_SERVICE' INTO if_stctm_task~pr_log->dummy ##NO_TEXT.
        if_stctm_task~pr_log->add_syst( ).

        MESSAGE e000 WITH 'For support open Fiori Apps Library;'  'search for OData service;' 'check SUPPORT for component' INTO if_stctm_task~pr_log->dummy ##NO_TEXT.
        if_stctm_task~pr_log->add_syst( ).

        MESSAGE e000 WITH 'Summary failed services (click icon)' INTO if_stctm_task~pr_log->dummy ##NO_TEXT.
