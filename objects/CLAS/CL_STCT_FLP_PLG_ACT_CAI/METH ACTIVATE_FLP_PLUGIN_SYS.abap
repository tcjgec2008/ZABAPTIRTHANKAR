  METHOD activate_flp_plugin_sys.
    ...
    SELECT SINGLE * FROM (/ui2/flpsetpa) INTO CORRESPONDING FIELDS OF @ls_flpsetpa WHERE plugin_id = @i_plugin_id.

    MESSAGE s000 WITH 'Activation for all client' '(/UI2/FLP_SYS_CONF)' INTO if_stctm_task~pr_log->dummy ##NO_TEXT.
    ...
    IF ls_flpsetpa IS NOT INITIAL.
      ...
      ELSE.
        ...
        IF i_overwrite = abap_true.
          ...
          IF i_act_state = ''.
            MESSAGE s000 WITH lv_msg 'ACTIVE <>' ls_flpsetpa-act_state INTO if_stctm_task~pr_log->dummy ##NO_TEXT.
            if_stctm_task~pr_log->add_syst( ).
          ELSE.
            MESSAGE s000 WITH lv_msg i_act_state '<>' ls_flpsetpa-act_state INTO if_stctm_task~pr_log->dummy ##NO_TEXT.
            if_stctm_task~pr_log->add_syst( ).
          ENDIF.
        ...
        ELSE.
          ...
          IF i_act_state = ''.
            MESSAGE w000 WITH lv_msg 'ACTIVE <>' ls_flpsetpa-act_state INTO if_stctm_task~pr_log->dummy ##NO_TEXT.
            if_stctm_task~pr_log->add_syst( ).
          ELSE.
            MESSAGE w000 WITH lv_msg i_act_state '<>' ls_flpsetpa-act_state INTO if_stctm_task~pr_log->dummy ##NO_TEXT.
            if_stctm_task~pr_log->add_syst( ).
          ENDIF.
    ...
    IF lv_insert = abap_true OR lv_update = abap_true.
      ...
      ELSE.
        ...
        ls_flpsetpa-plugin_id = i_plugin_id.
        ls_flpsetpa-act_state = i_act_state.
        ...
        wa_e071k-objname = '/UI2/FLPSETPA'.
        wa_e071k-tabkey   = ls_flpsetpa-plugin_id.
        wa_e071k-mastertype = wa_e071-object.
        wa_e071k-mastername = wa_e071-obj_name.
        wa_e071k-viewname = '/UI2/FLPSETPAV'.
        ...
        ELSE.
          ...
          IF lv_update = abap_true.
            ...
            " update table
            UPDATE (/ui2/flpsetpa) FROM ls_flpsetpa.

            IF i_act_state = ''.
              MESSAGE s000 WITH 'Updated FLP Plugin:' ls_flpsetpa-plugin_id '= ACTIVE' INTO if_stctm_task~pr_log->dummy ##NO_TEXT.
              if_stctm_task~pr_log->add_syst( ).
            ELSE.
              MESSAGE s000 WITH 'Updated FLP Plugin:' ls_flpsetpa-plugin_id '=' ls_flpsetpa-act_state INTO if_stctm_task~pr_log->dummy ##NO_TEXT.
              if_stctm_task~pr_log->add_syst( ).
            ENDIF.
          ...
          ELSE.
            ...
            " insert table
            INSERT (/ui2/flpsetpa) FROM ls_flpsetpa.

            IF i_act_state = ''.
              MESSAGE s000 WITH 'Created FLP Plugin:' ls_flpsetpa-plugin_id '= ACTIVE' INTO if_stctm_task~pr_log->dummy ##NO_TEXT.
              if_stctm_task~pr_log->add_syst( ).
            ELSE.
              MESSAGE s000 WITH 'Created FLP Plugin:' ls_flpsetpa-plugin_id '=' ls_flpsetpa-act_state INTO if_stctm_task~pr_log->dummy ##NO_TEXT.
              if_stctm_task~pr_log->add_syst( ).
            ENDIF.
