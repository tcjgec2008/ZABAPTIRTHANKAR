  METHOD if_stctm_bg_task~execute.
    ...
    DATA lv_act_state TYPE c LENGTH 8.
...
*****************************************
    ...
    IF i_check = 'X'.
      ...
      " all clients /UI2/FLP_SYS_CONF
      IF lv_sys = abap_true.

        IF lv_overwrite = abap_false.

          " check plugin state
          lv_plugin_id = 'CAI_PLUGIN'.
          lv_act_state = ''. " Active

          me->check_flp_plugin_sys(
            EXPORTING
              i_plugin_id = lv_plugin_id
              i_act_state = lv_act_state
            IMPORTING
              e_rc        =  lv_rc_ret
          ).
      ...
      " current client /UI2/FLP_CUS_CONF
      IF lv_cus = abap_true.
